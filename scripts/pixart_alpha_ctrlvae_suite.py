...
SUITE_ROOT = PIXART_ROOT / "results" / "full_700_ctrlvae_20260323"
...
EVAL_ROOT = DIFFSR_ROOT / "results" / "PixArtAlpha_ctrlvae"

PIXART_MODEL_PATH = REPO_ROOT / "checkpoints" / "hf_models" / "pixart-alpha-xl-2-512x512"
CONTROLVAE_PATH = REPO_ROOT / "checkpoints" / "Controlvae.pt"
...
RUN_SPECS = {
    "pixart_ctrl_700_surS": {
        "backbone": "pixart",
        "surrogate": "S",
        "controlvae": 1,
        "model_name": "S,E,R,D",
        "pretrained_diffusion_path": str(PIXART_MODEL_PATH),
        "encoder_weights": str(CONTROLVAE_PATH),
        "compared_to": "pixart_noctrl_700_surS|sd_ctrl_700_surS|sd_noctrl_700_surS|sr_ctrl_700_surS|sr_noctrl_700_surS",
    },
}
...
def run_output_name(run_id: str) -> str:
    ...
    controlvae = int(RUN_SPECS[run_id]["controlvae"])
    prefix = "ctrlvae" if controlvae else "noctrlvae"
    return f"PixArtAlpha_genimage_eval_1400_adv_{prefix}_sur{surrogate}"
...
def attack_args_for_run(run_id: str) -> dict[str, object]:
    ...
    args["encoder_weights"] = spec["encoder_weights"]
...
def run_attack(run_id: str) -> None:
    ...
    run_meta = {
        "run_id": run_id,
        "problem": "Run PixArt-Alpha + ControlVAE full GenImage-700 attack for surrogate S and export evaluation-ready dataroots.",
        "pipeline": [
            ...
            "DDIM denoise",
            "decode with ControlVAE NewDecoder",
        ...
        "candidate_modules": {
            "enabled": [
                ...
                "tokenizer",
                "text_encoder",
                "ControlVAE NewEncoder",
                "ControlVAE NewDecoder",
                "detectors E/R/D/S",
            ],
            "disabled": [],
        },
        ...
        "output_weights": str(CONTROLVAE_PATH),
...
def write_suite_spec(run_ids: list[str]) -> None:
    spec = {
        "problem": "Evaluate whether enabling ControlVAE on PixArt-Alpha improves the full GenImage-700 surrogate-S attack compared with the existing PixArt no-ControlVAE and SD/SR baselines.",
        "pipeline": [
            "load PixArt-Alpha backbone",
    ...
    lines = [
        "# PixArt-Alpha + ControlVAE full-700 suite spec",
        "",
        "## Problem",
        f"- {spec['problem']}",
        "",
    ...
    lines.extend(
        [
            ...
            f"- output weights: `{CONTROLVAE_PATH}`",
        ]
    )
...
def summarize_suite(run_ids: list[str]) -> list[dict[str, object]]:
    ...
    row = rows[0]
    md_lines = [
        "# PixArt-Alpha + ControlVAE full GenImage-700 suite (2026-03-23)",
        "",
        "## Problem",
        "- Evaluate the full 700-fake PixArt-Alpha + ControlVAE attack for surrogate S.",
        "",
        "## Outputs",
        ...
        "",
        "## Summary",
        f"- Whitebox ASR: `{row['run_id']}` = `{row['whitebox_asr']:.6f}`",
        f"- Blackbox mean ASR: `{row['run_id']}` = `{row['blackbox_mean_asr']:.6f}`",
        f"- PSNR: `{row['run_id']}` = `{row['psnr']:.6f}`",
        f"- Fourier L2 to real: `{row['run_id']}` = `{row['fourier_l2_to_real']:.6f}`",
        "",
        "## Summary CSV",
        f"- `{SUMMARY_CSV}`",
    ]
...
def materialize_eval_root(run_ids: list[str]) -> None:
    ...
    for run_id in run_ids:
        surrogate = str(RUN_SPECS[run_id]["surrogate"])
        out_dir = run_dir(run_id)
        copy_text_like(out_dir / f"adv_logits_sur{surrogate}.csv", EVAL_ROOT / f"adv_logits_sur{surrogate}.csv")
        copy_text_like(out_dir / "transfer_asr.csv", EVAL_ROOT / "transfer_asr.csv")
        copy_text_like(out_dir / "transfer_asr_by_subset.csv", EVAL_ROOT / "transfer_asr_by_subset.csv")
        copy_text_like(out_dir / "metrics" / "visual" / "visual_quality.csv", EVAL_ROOT / "visual_quality.csv")
        copy_text_like(
            out_dir / "metrics" / "visual" / "visual_quality_by_subset.csv",
            EVAL_ROOT / "visual_quality_by_subset.csv",
        )
        copy_text_like(
            out_dir / "metrics" / "frequency" / "frequency_metrics.csv",
            EVAL_ROOT / "frequency_metrics.csv",
        )

    meta = {
        "problem": "Full GenImage-700 PixArt-Alpha + ControlVAE surrogate-S evaluation.",
