...
DATE_TAG = "20260409"
RESULT_ROOT = PIXART_ROOT / "results" / f"cross_attn_full700_{DATE_TAG}"
...
RUN_SPECS = {
    "E": {
        "run_id": f"pixart_crossattn_full700_surE_{DATE_TAG}",
        "record_id": f"pixart_crossattn_full700_surE_{DATE_TAG}",
        "experiment_name": "PixArt-Alpha cross-attn full700 surrogate E",
        "model_name": "E,R,D,S",
        "compared_to": "pixart_selfattn_full700_surE_g1p0_20260406|pixart_crossattn_smoke35_surE_20260329|pixart_noctrl_700_surE",
    },
    "R": {
        "run_id": f"pixart_crossattn_full700_surR_{DATE_TAG}",
        "record_id": f"pixart_crossattn_full700_surR_{DATE_TAG}",
        "experiment_name": "PixArt-Alpha cross-attn full700 surrogate R",
        "model_name": "R,E,D,S",
        "compared_to": "pixart_selfattn_full700_surR_g1p0_20260406|pixart_crossattn_smoke35_surR_20260329|pixart_noctrl_700_surR",
    },
    "D": {
        "run_id": f"pixart_crossattn_full700_surD_{DATE_TAG}",
        "record_id": f"pixart_crossattn_full700_surD_{DATE_TAG}",
        "experiment_name": "PixArt-Alpha cross-attn full700 surrogate D",
        "model_name": "D,E,R,S",
        "compared_to": "pixart_selfattn_full700_surD_g1p0_20260406|pixart_crossattn_smoke35_surD_20260329|pixart_noctrl_700_surD",
    },
    "S": {
        "run_id": f"pixart_crossattn_full700_surS_{DATE_TAG}",
        "record_id": f"pixart_crossattn_full700_surS_{DATE_TAG}",
        "experiment_name": "PixArt-Alpha cross-attn full700 surrogate S",
        "model_name": "S,E,R,D",
        "compared_to": "pixart_selfattn_full700_surS_g1p0_20260406|pixart_crossattn_smoke35_surS_20260326|pixart_noctrl_700_surS",
    },
}
...
def output_dataroot(surrogate: str) -> Path:
    return ATTACK_DATASETS_ROOT / f"PixArtAlpha_full700_adv_crossattn_sur{surrogate}_{DATE_TAG}"


def attack_log_path(surrogate: str) -> Path:
    model_name = RUN_SPECS[surrogate]["model_name"].replace(",", "")
    return PIXART_ROOT / "exp" / "results" / f"attack_{model_name}0_sur{surrogate}.log"
...
def run_attack(surrogate: str) -> None:
    ...
    write_json(
        out_dir / "run_meta.json",
        {
            ...
            "problem": "Promote the smoke35 PixArt cross-attn tradeoff row to a formal full700 result on the same surrogate.",
            "pipeline": [
                "PixArt transformer backbone",
                "DDIM inversion",
                "pseudo-label text forwarding",
                "LAO latent optimization with cross-attn regularization",
                "export transfer / visual / frequency metrics",
            ],
            "candidate_modules": {
                "enabled": ["PixArtAlpha transformer", "pseudo-label prompt embeddings", "cross-attn regularizer"],
                "disabled": ["ControlVAE", "self-attn regularizer", "frequency loss"],
            },
            "metrics": [
                "whitebox_asr",
                "blackbox_mean_asr",
...
def collect_summary(surrogate: str) -> dict[str, object]:
    ...
    row = {
        ...
        "controlvae": 0,
        "prompt_source": "full700_pseudo_labels",
        "cross_attn_weight": BASE_ATTACK_ARGS["cross_attn_loss_weight"],
...
def build_record_row(summary: dict[str, object]) -> dict[str, str]:
    ...
    return {
        ...
        "family": "ablation_cross_attn_full700",
        ...
        "lr": "1e-3",
        "lambda_perc": "0.5",
        "primary_change": "cross_attn_only",
        ...
        "new_information": (
            f"Formal full700 PixArt cross-attn row for surrogate {surrogate}. "
            "This tests whether the cross-attn route keeps its visual-quality advantage over the matched self-attn full700 row "
            "under the same no-ControlVAE full-dataset setting."
        ),
...
def main() -> int:
    ...
    parser.add_argument("--surrogates", default="E,R,D,S", help="Comma-separated subset of E,R,D,S")
